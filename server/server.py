...
parser.add_argument("--leaky", type=float, default = 0.90, help = 'print interval(default : 1)')
...
def get_entry():
    ...
    while True:
        ...
        reward = [0] 
        S_core0_IPC = (float(entry[0][2]/(entry[0][3])))
        S_core1_IPC = (float(entry[0][16]/(entry[0][17])))
        S_core2_IPC = (float(entry[0][30]/(entry[0][31])))
        S_core3_IPC = (float(entry[0][44]/(entry[0][45])))
        S_core_IPC = (S_core0_IPC+S_core1_IPC+S_core2_IPC+S_core3_IPC)/4
        
        NS_core0_IPC = (float(entry[1][2]/(entry[1][3])))
        NS_core1_IPC = (float(entry[1][16]/(entry[1][17])))
        NS_core2_IPC = (float(entry[1][30]/(entry[1][31])))
        NS_core3_IPC = (float(entry[1][2]/(entry[1][45])))
        ...
        if not np.isnan(diff):
            reward[0] = diff
        total_reward += diff
        print("reward", reward, total_reward, memory.size())
...
def train():
    ...
    while True:
        if(memory.size() > batch_size):
            print("training in progress", memory.size())
